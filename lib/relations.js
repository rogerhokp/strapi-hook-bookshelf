...
module.exports = {
      ...
      params.parseRelationships === false
        ...
        : Object.keys(removeUndefinedKeys(params.values)).reduce(
          (acc, current) => {
            ...
            relationUpdates.push(module.exports.fireBeforeUpdate(assocModel, transformToArrayID(
              response[current],
              association
            ).map(id => id.toString()), params, association, this.collectionName));
            ...
            switch (association.nature) {
              ...
              case 'oneToOne': {
                ...
                if (_.isNull(property)) {
                  ...
                  relationUpdates.push(module.exports.fireAfterUpdate(assocModel, params.id, params, association.alias, this.collectionName));
  ...
  async fireBeforeUpdate(assocModel, ids, params, association, updateFrom) {

    const fn = _.get(assocModel, ['beforeRelationUpdate']);
    if (fn && _.isFunction(fn)) {
      await fn(ids, updateFrom, params.id);
    }
  },
  async fireAfterUpdate(assocModel, ids, params, alias, updateFrom) {

    const fn = _.get(assocModel, ['afterRelationUpdate']);
    if (fn && _.isFunction(fn)) {
      await fn(
        alias ? _.get(params, ['values', alias], null) : null,
        updateFrom,
        ids
      );
    }
